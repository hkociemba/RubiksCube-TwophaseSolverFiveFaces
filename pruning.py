...
def create_phase1_prun_table():
    ...
    if not path.isfile(fname):
        ...
        cc = cb.CubieCube()
        fs_sym = ar.array('H', [0] * defs.N_FLIPSLICE_CLASS_2cv)
        for i in range(defs.N_FLIPSLICE_CLASS_2cv):
            if (i + 1) % 1000 == 0:
        ...
        while done != total:
            ...
            if depth < 9:
                ...
                mult = 1
            idx = 0
            for fs_classidx in range(defs.N_FLIPSLICE_CLASS_2cv):
                ...
                while twist < defs.N_TWIST:
                    ...
                    if match:
                        ...
                        for m in enums.Move:
                            twist1 = mv.twist_move[15 * twist + m]  # defs.N_MOVE = 15
                            flip1 = mv.flip_move[15 * flip + m]
                            slice1 = mv.slice_sorted_move[360 * slice_ + m] // 24  # defs.N_PERM_4 = 24, 15*24 = 360
                            flipslice1 = (slice1 << 11) + flip1
                            fs1_classidx = sy.flipslice_classidx[flipslice1]
                            fs1_sym = sy.flipslice_sym[flipslice1]
                            twist1 = sy.twist_conj[(twist1 << 2) + fs1_sym]
                                ...
                                if get_flipslice_twist_depth3(idx1) == 3:  # entry not yet filled
                                    ...
                                    if sym != 1:
                                        for j in range(1, 4):
                                            sym >>= 1
                                            if sym % 2 == 1:
                                                twist2 = sy.twist_conj[(twist1 << 2) + j]
...
def create_phase2_prun_table():
    ...
    if not path.isfile(fname):
        ...
        cc = cb.CubieCube()
        c_sym = ar.array('H', [0] * defs.N_CORNERS_CLASS_2cv)
        for i in range(defs.N_CORNERS_CLASS_2cv):
            if (i + 1) % 1000 == 0:
                print('.', end='', flush=True)
            rep = sy.corner_rep[i]
            cc.set_corners(rep)
            for s in range(defs.N_SYM_C2v):
                ss = cb.CubieCube(sy.symCube[s].cp, sy.symCube[s].co, sy.symCube[s].ep, sy.symCube[s].eo)  # copy cube
        ...
        depth = 0
        print('depth:', depth, 'done: ' + str(done) + '/' + str(total))
        while depth < 11:  # we fill the table only to depth 11 + 1
            depth3 = depth % 3
            idx = 0
            mult = 2
            if depth > 9:
                mult = 1
            for c_classidx in range(defs.N_CORNERS_CLASS_2cv):
                ...
                while ud_edge < defs.N_UD_EDGES:
                    ...
                    if get_corners_ud_edges_depth3(idx) == depth3:
                        ...
                        for m in (enums.Move.U1, enums.Move.U2, enums.Move.U3, enums.Move.R2, enums.Move.F2,
                                  enums.Move.D1, enums.Move.D2, enums.Move.D3, enums.Move.L2):
                            ud_edge1 = mv.ud_edges_move[15 * ud_edge + m]
                            corner1 = mv.corners_move[15 * corner + m]
                            c1_classidx = sy.corner_classidx[corner1]
                            c1_sym = sy.corner_sym[corner1]
                            ud_edge1 = sy.ud_edges_conj[(ud_edge1 << 2) + c1_sym]
                            ...
                            if get_corners_ud_edges_depth3(idx1) == 3:  # entry not yet filled
                                ...
                                if sym != 1:
                                    for j in range(1, 4):
                                        sym >>= 1
                                        if sym % 2 == 1:
                                            ud_edge2 = sy.ud_edges_conj[(ud_edge1 << 2) + j]
        ...
        print('remaining unfilled entries have depth >=' + str(depth + 1))
        fh = open(fname, "wb")
            ...
            for corners in range(defs.N_CORNERS):
                for slice_ in range(defs.N_PERM_4):
                    if cornslice_depth[defs.N_PERM_4 * corners + slice_] == depth:
                        for m in (enums.Move.U1, enums.Move.U2, enums.Move.U3, enums.Move.R2, enums.Move.F2,
                                  enums.Move.D1, enums.Move.D2, enums.Move.D3, enums.Move.L2):
                            corners1 = mv.corners_move[defs.N_MOVE * corners + m]
                            slice_1 = mv.slice_sorted_move[defs.N_MOVE * slice_ + m]
...
# array distance gives the new distance from the old_distance i and the new_distance_mod3 j. ########################
