...
class SolverThread(thr.Thread):
    ...
    def __init__(self, cb_cube, rot, inv, ret_length, timeout, start_time, solutions, terminated, shortest_length):
        ...
        :param rot: Rotates the  cube 90° * rot along the FB-axis before applying the two-phase-algorithm
    ...
    def search_phase2(self, corners, ud_edges, slice_sorted, dist, togo_phase2):
        ...
        if togo_phase2 == 0:
            ...
            if len(self.solutions) == 0 or (len(self.solutions[-1]) > len(man)):
                ...
                man[:] = [en.Move(sy.conj_move[m, 38 * self.rot]) for m in man]
            ...
            for m in en.Move:
                ...
                corners_new = mv.corners_move[15 * corners + m]
                ud_edges_new = mv.ud_edges_move[15 * ud_edges + m]
                slice_sorted_new = mv.slice_sorted_move[15 * slice_sorted + m]

                classidx = sy.corner_classidx[corners_new]
                sym = sy.corner_sym[corners_new]
                dist_new_mod3 = pr.get_corners_ud_edges_depth3(
                    40320 * classidx + sy.ud_edges_conj[(ud_edges_new << 2) + sym])
    ...
    def search(self, flip, twist, slice_sorted, dist, togo_phase1):
        ...
        if togo_phase1 == 0:  # phase 1 solved
            ...
            if m in [en.Move.R3, en.Move.F3, en.Move.L3]:  # phase 1 solution come in pairs
                corners = mv.corners_move[15 * self.cornersave + m - 1]  # apply R2, F2, L2 ord B2 on last ph1 solution
            else:
                corners = self.co_cube.corners
                for m in self.sofar_phase1:  # get current corner configuration
                    corners = mv.corners_move[15 * corners + m]
                self.cornersave = corners

            # new solution must be shorter and we do not use phase 2 maneuvers with length > 13 - 1 = 12
            togo2_limit = min(self.shortest_length[0] - len(self.sofar_phase1), 13)
            ...
            for m in self.sofar_phase1:
                u_edges = mv.u_edges_move[15 * u_edges + m]
                d_edges = mv.d_edges_move[15 * d_edges + m]
            ...
            for m in en.Move:
                ...
                flip_new = mv.flip_move[15 * flip + m]  # N_MOVE = 15
                twist_new = mv.twist_move[15 * twist + m]
                slice_sorted_new = mv.slice_sorted_move[15 * slice_sorted + m]
                ...
                dist_new_mod3 = pr.get_flipslice_twist_depth3(2187 * classidx + sy.twist_conj[(twist_new << 2) + sym])
                dist_new = pr.distance[3 * dist + dist_new_mod3]
    ...
    def run(self):
        ...
        elif self.rot == 1:  # conjugation by 90° rotation about FB axis
            cb = cubie.CubieCube(sy.symCube[36].cp, sy.symCube[36].co, sy.symCube[36].ep, sy.symCube[36].eo)
            cb.multiply(self.cb_cube)
            cb.multiply(sy.symCube[38])
...
def solve(cubestring, max_length=20, timeout=3):
    ...
    tr = range(4)
    for i in tr:
        th = SolverThread(cc, i % 2, i // 2, max_length, timeout, s_time, solutions, terminated, [999])
