...
class CoordCube:
    ...
    def get_depth_phase1(self):
        ...
        depth_mod3 = pr.get_flipslice_twist_depth3(N_TWIST * classidx + sy.twist_conj[(twist << 2) + sym])
        ...
        while flip != SOLVED or slice_ != SOLVED or twist != SOLVED:
            ...
            for m in enums.Move:
                ...
                if pr.get_flipslice_twist_depth3(N_TWIST * classidx1 + sy.twist_conj[(twist1 << 2) + sym]) == depth_mod3 - 1:
                    depth += 1
                    twist = twist1
                    flip = flip1
                    slice_ = slice1
    ...
    def get_depth_phase2(corners, ud_edges):
        ...
        depth_mod3 = pr.get_corners_ud_edges_depth3(N_UD_EDGES * classidx + sy.ud_edges_conj[(ud_edges << 2) + sym])
        ...
        while corners != SOLVED or ud_edges != SOLVED:
            if depth_mod3 == 0:
                depth_mod3 = 3
            # only iterate phase 2 moves, no B2
            for m in (enums.Move.U1, enums.Move.U2, enums.Move.U3, enums.Move.R2, enums.Move.F2, enums.Move.D1,
                ...
                if pr.get_corners_ud_edges_depth3(N_UD_EDGES * classidx1 + sy.ud_edges_conj[(ud_edges1 << 2) + sym]) ==\
                        depth_mod3 - 1:
                    depth += 1
                    corners = corners1
                    ud_edges = ud_edges1
